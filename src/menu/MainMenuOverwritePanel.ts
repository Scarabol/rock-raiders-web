...
export class MainMenuOverwritePanel extends MainMenuBaseItem {
    ...
    constructor(parentWidth: number, parentHeight: number) {
        ...
        this.zIndex = 50
        ResourceManager.bitmapFontWorkerPool.createTextImage('Interface/Fonts/MbriefFont2.bmp', ResourceManager.configuration.menu.overwrite.title, dialogCfg.titleWindow.w).then((img) => {
            this.titleImg = img
    ...
    setIndex(savegameIndex: number) {
        const text = ResourceManager.configuration.menu.overwrite.text.replaceAll('%d', savegameIndex.toString())
        ResourceManager.bitmapFontWorkerPool.createTextImage('Interface/Fonts/MbriefFont.bmp', text, ResourceManager.configuration.dialog.textWindow.w).then((img) => {
            this.textImg = img
            const dialogCfg = ResourceManager.configuration.dialog
