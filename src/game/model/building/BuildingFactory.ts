    ...
    static createBuildingFromType(entityType: EntityType, sceneMgr: SceneManager, entityMgr: EntityManager): BuildingEntity {
        switch (entityType) {
            case EntityType.TOOLSTATION:
                return new BuildingEntityBuilder(sceneMgr, entityMgr, entityType, ResourceManager.configuration.stats.Toolstation, 'Buildings/Toolstation/Toolstation.ae')
                    .addTeleport(new RaiderOnlyTeleport()).build()
            case EntityType.TELEPORT_PAD:
                return new BuildingEntityBuilder(sceneMgr, entityMgr, entityType, ResourceManager.configuration.stats.TeleportPad, 'Buildings/Teleports/Teleports.ae')
                    .addTeleport(new SmallTeleport()).build()
            case EntityType.DOCKS:
                return new BuildingEntityBuilder(sceneMgr, entityMgr, entityType, ResourceManager.configuration.stats.Docks, 'Buildings/Docks/Docks.ae')
                    .primaryPowerPath(0, -1).waterPathSurface(0, 1).build()
            case EntityType.POWER_STATION:
                return new BuildingEntityBuilder(sceneMgr, entityMgr, entityType, ResourceManager.configuration.stats.Powerstation, 'Buildings/Powerstation/Powerstation.ae')
                    .secondaryBuildingPart(-1, 0).dropAction(RaiderActivity.Deposit).build()
            case EntityType.BARRACKS:
                return new BuildingEntityBuilder(sceneMgr, entityMgr, entityType, ResourceManager.configuration.stats.Barracks, 'Buildings/Barracks/Barracks.ae')
                    .build()
            case EntityType.UPGRADE:
                return new BuildingEntityBuilder(sceneMgr, entityMgr, entityType, ResourceManager.configuration.stats.Upgrade, 'Buildings/Upgrade/Upgrade.ae')
                    .build()
            case EntityType.GEODOME:
                return new BuildingEntityBuilder(sceneMgr, entityMgr, entityType, ResourceManager.configuration.stats.Geodome, 'Buildings/Geo-dome/Geo-dome.ae')
                    .removePrimaryPowerPath().secondaryBuildingPart(0, 1).build()
            case EntityType.ORE_REFINERY:
                return new BuildingEntityBuilder(sceneMgr, entityMgr, entityType, ResourceManager.configuration.stats.OreRefinery, 'Buildings/OreRefinery/OreRefinery.ae')
                    .primaryPowerPath(0, 2).secondaryBuildingPart(0, 1).dropAction(RaiderActivity.Deposit).build()
            case EntityType.GUNSTATION:
                return new BuildingEntityBuilder(sceneMgr, entityMgr, entityType, ResourceManager.configuration.stats.GunStation, 'Buildings/gunstation/gunstation.ae')
                    .removePrimaryPowerPath().build()
            case EntityType.TELEPORT_BIG:
                return new BuildingEntityBuilder(sceneMgr, entityMgr, entityType, ResourceManager.configuration.stats.TeleportBIG, 'Buildings/BIGTeleport/BIGTeleport.ae')
