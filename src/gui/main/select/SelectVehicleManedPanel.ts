...
export class SelectVehicleManedPanel extends SelectBasePanel {
    ...
    constructor(parent: BaseElement, onBackPanel: Panel) {
        ...
        const unloadVehicleItem = this.addMenuItem(OffscreenCache.configuration.interfaceImages, 'Interface_MenuItem_UnLoadVehicle')
        unloadVehicleItem.isDisabled = () => this.noVehicleWithCarriedItems
        unloadVehicleItem.onClick = () => this.publishEvent(new VehicleUnload())
        this.addMenuItem(OffscreenCache.configuration.interfaceImages, 'Interface_MenuItem_VehiclePickUp')
        this.upgradeItem = this.addMenuItem(OffscreenCache.configuration.interfaceImages, 'Interface_MenuItem_UpgradeVehicle')
        this.upgradeItem.isDisabled = () => !this.hasUpgradeSite
        const leaveVehicleItem = this.addMenuItem(OffscreenCache.configuration.interfaceImages, 'Interface_MenuItem_GetOut')
